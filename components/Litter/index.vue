    ...
    <template #image-overlay>
      ...
      >
        <ul v-if="isPuppy(currentItem) && currentItem.price" class="grid w-full max-w-sm gap-3 px-2 text-sm sm:text-base">
          <li class="text-center font-display text-2xl font-normal leading-none text-foreground">{{ currentItem?.name }}</li>
          <li class="flex items-center justify-between gap-4 border-t border-border pt-3 nowrap"><b>{{ site.litter.nickname }}</b><span>{{ currentItem.nickname }}</span></li>
        ...
        <div v-else class="flex w-full items-center justify-center px-2 text-center">
          <NuxtLink
            ...
            :to="currentItemPath"
            class="font-display text-2xl font-normal leading-none text-foreground underline decoration-primary-border underline-offset-8 transition hover:text-primary hover:decoration-primary"
          >
            {{ currentItem?.name }}
          </NuxtLink>
          <p v-else class="font-display text-2xl font-normal leading-none text-foreground">{{ currentItem?.name }}</p>
        </div>
      </CarouselCard>
    </template>
